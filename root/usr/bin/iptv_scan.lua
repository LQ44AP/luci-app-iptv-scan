...
-- ================= 文件锁逻辑 =================
-- 获取当前 Lua 进程的真实 PID
-- 说明：io.open 在 Lua 自身进程内执行 open(2)，不会 fork 子进程，
--       所以 /proc/self/stat 中的 self 就是当前的 Lua 进程。
--       这与 io.popen("sh -c 'echo $$'") 完全不同，后者拿到的是 sh 子进程 PID。
...
-- 通过 /proc/<pid>/stat 是否存在来判断进程是否存活
-- 相比 os.execute("kill -0 ...") 的优点是：不依赖 shell，也不受
-- Lua 5.1(返回 0/1) 与 5.2+(返回 true/nil,code) 差异影响
...
local function acquire_lock()
    ...
    if f then
        ...
        f:close()
        if pid_str then
            pid_str = pid_str:gsub("%s+", "")
            if pid_str:match("^%d+$") and is_pid_alive(tonumber(pid_str)) then
                return false
            end
        end
        -- 旧锁文件所属进程已死 / 内容无效：清理后重新获取
        os.remove(LOCK_FILE)
    end

    local my_pid = get_own_pid()
    if not my_pid then
        -- 极端情况下拿不到自身 PID（如 /proc 未挂载）：降级为无锁运行，
        -- 避免把自己永久卡死在“扫描任务已在运行”里
        log("[警告] 无法获取当前进程 PID，文件锁降级为无锁运行")
        return true
    end

    f = io.open(LOCK_FILE, "w")
    if not f then return false end
    f:write(tostring(my_pid))
    f:close()
    return true
end
...
local function run_scan()
    ...
    for _, task in ipairs(validated_tasks) do
        ...
        for i = 1, 255 do
            ...
            -- 关键修复：允许多次快速复用同一端口。
            -- 否则上一轮 socket 关闭后端口可能处于 TIME_WAIT，
            -- 或与下一次循环的新 socket 冲突，导致 setsockname 静默失败、
            -- 整个网段被跳过。必须在 setsockname 之前设置。
            udp:setoption("reuseaddr", true)

            local ok_bind, bind_err = udp:setsockname("0.0.0.0", port)
            if not ok_bind then
                log(string.format("  [!] 绑定端口 %d 失败，跳过 %s: %s",
                    port, target_ip, tostring(bind_err)))
                udp:close()
            else
                local ok_mem, mem_err = udp:setoption("ip-add-membership",
                    { multiaddr = target_ip, interface = net.ip })
                if not ok_mem then
                    log(string.format("  [!] 加入组播 %s 失败: %s",
                        target_ip, tostring(mem_err)))
                    udp:close()
                else
